...
public class HttpTaskManagerSubtasksTest {
    ...
    void testCreateGetDeleteSubtask() throws Exception {
        ...
        HttpRequest postEpic = HttpRequest.newBuilder().uri(URI.create("http://localhost:3000/epics")).header("Content-Type", "application/json").POST(HttpRequest.BodyPublishers.ofString(epicJson)).build();
        HttpResponse<Void> epicResp = client.send(postEpic, HttpResponse.BodyHandlers.discarding());
        assertEquals(201, epicResp.statusCode());

        HttpRequest getEpics = HttpRequest.newBuilder().uri(URI.create("http://localhost:3000/epics")).GET().build();
        ...
        HttpRequest post = HttpRequest.newBuilder().uri(URI.create("http://localhost:3000/subtasks")).header("Content-Type", "application/json").POST(HttpRequest.BodyPublishers.ofString(json)).build();
        HttpResponse<Void> postResp = client.send(post, HttpResponse.BodyHandlers.discarding());
        assertEquals(201, postResp.statusCode());

        HttpRequest getAll = HttpRequest.newBuilder().uri(URI.create("http://localhost:3000/subtasks")).GET().build();
        ...
        HttpRequest getOne = HttpRequest.newBuilder().uri(URI.create("http://localhost:3000/subtasks?id=" + id)).GET().build();
        ...
        HttpRequest delete = HttpRequest.newBuilder().uri(URI.create("http://localhost:3000/subtasks?id=" + id)).DELETE().build();
